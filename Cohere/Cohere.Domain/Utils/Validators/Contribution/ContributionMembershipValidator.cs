...
{
    public class ContributionMembershipValidator : AbstractValidator<ContributionMembershipViewModel>
    {
        public ContributionMembershipValidator()
        {
            Include(new ContributionBaseValidator());

            RuleFor(c => c.Type).Cascade(CascadeMode.StopOnFirstFailure)
                .NotEmpty().WithMessage("{PropertyName} must not be empty")
                .Equal(nameof(ContributionMembership)).WithMessage("{PropertyName} must be one of predefined contribution types");
